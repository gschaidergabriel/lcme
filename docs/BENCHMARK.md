# LCME Memory: Empirical Benchmark for Local AI Agents
...
LCME is designed for this scenario: long-term memory that runs alongside a small model without competing for the same compute.

We benchmark LCME Memory v1.0.0 against Mem0 v1.0.7 head-to-head. Graphiti v0.28.2 and Letta v0.16.6 could not be benchmarked — both require external infrastructure (Neo4j server / Letta server) that contradicts the local-first constraint.

**Head-to-head results:**

| Metric | **LCME** | Mem0 | Graphiti | Letta |
|--------|-----------|------|----------|-------|
...
|--------|---------|--------------------|-----------------------|
| **LCME** | v1.0.0 | Yes | None (SQLite + PyTorch CPU) |
...
Graphiti and Letta could not be benchmarked — both require external infrastructure that prevents fully local operation. This is a valid finding: **LCME is the only system that runs with zero external dependencies.**
...
**LCME outperforms Mem0 on all retrieval quality metrics.** This was not the case before the v1.0.0 retrieval fix (see Section 3.6). The improvement comes from three fixes: removing entity nodes from the vector store, stop-word filtering for FTS queries, and rank-based RRF fusion with correct weight calibration.

Mem0's retrieval quality is strong (P@5 0.867) thanks to LLM-based extraction that produces clean, semantic memory representations. LCME achieves higher precision through multi-signal retrieval fusion (FTS keywords + vector semantics + graph structure) rather than relying on extraction quality alone.
...
LCME ingests a memory in 28ms. Mem0 takes nearly 12 seconds per item because every ingestion passes through a full LLM inference call (Qwen-3B on CPU). With a cloud API (GPT-4, Claude), Mem0 ingestion would be faster (~1-3s) but introduces cost and dependency.

**For always-on agents:** An agent processing 1,000 turns/day spends ~28 seconds on LCME ingestion vs ~3.3 hours on Mem0.
...
Mem0 is ~1.3x faster on retrieval because it performs a single vector ANN lookup against ChromaDB. LCME does multi-signal retrieval: FTS5 keyword search, vector cosine similarity, RRF rank fusion, entity filtering, and neural weight computation. Both are well within interactive latency (<20ms).
...
LCME uses 5.5x less RAM. Mem0's 93.4 MB includes ChromaDB's in-memory index (not counting the LLM's ~6-8 GB when loaded for ingestion).

### 3.5 Persistence and Contradiction

- **Cross-session persistence**: LCME verified. Shutdown and restart preserves all data.
- **Contradiction detection**: LCME verified. "Alice works at Google" → "Alice now works at Microsoft" → query returns Microsoft.

### 3.6 Historical Note: Pre-Fix Retrieval

LCME v1.0.0 initially shipped with a retrieval scoring bug that made results near-random (P@5=0.40, MRR=0.24). Three compounding issues were identified and fixed:
...
### 4.1 Why Only LCME Has Numbers

| System | Benchmarked? | Reason |
|--------|-------------|--------|
| LCME | Yes | Runs fully local, zero dependencies |
...
```
Feature                              LCME    Mem0     Graphiti   Letta
──────────────────────────────────────────────────────────────────────────
STORAGE
...
LCME's FTS stop-word filtering, temporal detection, emotion detection, and negation detection support 8 languages:
...
If you are running 70B+ models on multi-GPU servers with API budgets, Mem0 with a cloud LLM is a reasonable choice. If you are running a 3B model on a laptop and need memory that doesn't kill your system, LCME is built for you.
...
3. **Contradiction handling is 70%, not 100%.** LCME does not explicitly invalidate old facts. Both old and new facts coexist and are ranked by keyword/semantic match. Temporal validity (Graphiti-style) would improve this.
